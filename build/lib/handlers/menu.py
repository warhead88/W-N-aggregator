...
async def open_menu(message: types.Message, session: AsyncSession):
...
async def process_weather_place(message: types.Message, state: FSMContext, session: AsyncSession):
    place = message.text
    ...
    if user:
        user.place = place
        await message.answer(f"Место: {html.escape(place)}; было успешно задано как ваше основное.", parse_mode="HTML")

    await state.clear()

@router.message(News.waiting_for_text)
async def process_news_query(message: types.Message, state: FSMContext, session: AsyncSession):
    query = message.text
    ...
    if user:
        user.query = query
        await message.answer(f"Ключевые слова для поиска: {html.escape(query)}; были успешно заданы.", parse_mode="HTML")

    await state.clear()

@router.message(Count.waiting_for_text)
async def process_news_count(message: types.Message, state: FSMContext, session: AsyncSession):
    try:
...
async def change_sub(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession):
